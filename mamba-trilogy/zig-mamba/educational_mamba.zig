    ...
    pub fn init(allocator: Allocator, config: MambaConfig) !Self {
        ...
        for (0..config.d_inner) |i| {
            for (0..config.d_state) |j| {
                const val = -math.exp(random.float(f32) * @log(@as(f32, @floatFromInt(config.d_state))));
                A_log[i * config.d_state + j] = @log(-val);
            }
        }
        ...
        for (0..seq_len) |t| {
            for (0..batch_size) |b| {
                ...
                for (0..cfg.d_inner) |i| {
                    ...
                    for (0..cfg.d_state) |j| {
                        const A_val = @exp(self.A_log[i * cfg.d_state + j]);
...
pub fn benchmark_mamba(allocator: Allocator) !void {
    ...
    for (test_configs) |test_config| {
        ...
        print("   ⚡ Performance: {d:.2}ms ({d:.0} tokens/sec)\n", .{ elapsed, tokens_per_sec });
