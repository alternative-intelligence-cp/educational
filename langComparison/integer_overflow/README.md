...
This is not a theoretical question. Integer overflow has caused real security vulnerabilities, wrong calculations, and crashed systems. The famous Nitpickne 5 rocket explosion in 1996 was caused by an unchecked integer conversion.
...
# Nitpick
nitpickc overflow.npk -o overflow_nitpick && ./overflow_nitpick
```
...
| Nitpick | `2147483647` | LLVM `add nsw` elides the +1 entirely — **optimizer changed the result** |

### Nitpick's row is the most interesting one

Nitpick compiles to LLVM IR. Integer arithmetic emits `add nsw i32` — the `nsw` ("no signed wrap") flag tells LLVM the programmer *guarantees* this addition will not overflow. When it does anyway, LLVM treats it as undefined behavior and **removes the operation entirely**, keeping the original value.
...
| Nitpick | `0` (defined — LLVM unsigned arithmetic wraps) |
...
The most dangerous overflow bugs are **silent**: no crash, no exception, just a wrong value. In C (and currently Nitpick), a signed overflow in a path the optimizer decides was UB can produce wrong results, wrong branch decisions, or corrupted data — with no warning.
...
Nitpick's planned fix: safe arithmetic methods (`checked_add`, `wrapping_add`) matching Rust's approach.
