...
Side-by-side implementations of the same programs in **C**, **Python**, **Rust**, and **Nitpick**.
...
| **Nitpick** | Systems (in development) | Fixed arrays + arena allocator | `Result<T>` (partial) | ≈ C (goal) |
...
**Key differences:** C uses return flags + output pointers. Python uses exceptions that unwind the stack. Rust uses `Result<T,E>` with the `?` operator for zero-friction propagation. Nitpick currently uses sentinel values (C-style) — explicit `fail()` / `Err` syntax is planned.
...
**Key differences:** C and Rust give direct, fast array access. Python is readable but much slower at large N. Nitpick requires a fixed compile-time array size (a current limitation — dynamic allocation via the arena allocator is available but not yet as ergonomic). All four produce identical output.
...
Nitpick is a systems language under active development. These programs are both educational comparisons *and* real-world tests of the language as it evolves. Where Nitpick's current implementation differs from its design goals, the README in each subdirectory says so explicitly.
